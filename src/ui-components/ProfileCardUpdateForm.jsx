...
export default function ProfileCardUpdateForm(props) {
  const {
    id: idProp,
    profileCard: profileCardModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = profileCardRecord
      ? { ...initialValues, ...profileCardRecord }
      : initialValues;
  ...
  const [profileCardRecord, setProfileCardRecord] =
    React.useState(profileCardModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getProfileCard.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getProfileCard
        : profileCardModelProp;
      setProfileCardRecord(record);
    };
    queryData();
  }, [idProp, profileCardModelProp]);
  React.useEffect(resetStateValues, [profileCardRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      {...getOverrideProps(overrides, "ProfileCardUpdateForm")}
      {...rest}
    >
      <TextField
        label="First name"
      ...
      <TextField
        ...
        onChange={(e) => {
          ...
          if (errors.classesEnrolled?.hasError) {
            runValidationTasks("classesEnrolled", value);
          }
          setClassesEnrolled(value);
        }}
        onBlur={() => runValidationTasks("classesEnrolled", classesEnrolled)}
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || profileCardModelProp)}
